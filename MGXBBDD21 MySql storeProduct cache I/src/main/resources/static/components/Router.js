      ...
      // El marron
         ...
         const   nextPage = document.getElementById("botonNext").dataset.valor,
                    cacheLength = +getState().data.content.length,
                     ...
                     alert("cacheInicio:" + cacheInicio + "cacheFinal:" + cacheFinal+ "cacheLength:" + cacheLength)
          ...
          if (cacheLength >   (cacheFinal + visorSize)) {
            ...
            alert("nextPage: " + nextPage + "cacheSize: " +getState().cacheSize);
            callApiRest(uri+"page="+ (nextPage-1)+ "&size="+getState().cacheSize );
...
const render = function(showcase){
   let html = "";
   setState({visorSize: lS.getItem("visorSize"),activePage : lS.getItem("activePage"),cacheSize : lS.getItem("cacheSize")});
...
const  callApiRest = async function(uri){  
 ...
 if(!hash || hash == "#/"){     
     callApiRest("http://localhost:8085/storerest/?page=0&size=4");    
