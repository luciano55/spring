...
export async function Router(){
  ...
  $("div_menu_page").addEventListener("click",async (e)=>{
       let uri =  "http://localhost:8085/storerest/?";
        ...
        if(pageActive>0){
         // Ver si está en la cache
                 // cococer la cache activa y la nueva
          // sino
              let newPageToTransferred =  Math.ceil(((ageActive) * visorSize)/(visorSize*2)) -1;
         ...
         const nextPage = +document.getElementById("botonNext").dataset.valor;
          ...
          if (cacheLength >   (cacheFinal  + visorSize  ) || cacheLength < cacheSize )
          {
               ...
               if(cacheLength < cacheSize ) {
                        //setState({cacheInicio : cacheFinal +1, cacheFinal: cacheFinal +                 Math.floor(cacheLength / visorSize) ,activePageCache: activePageCache +1})
